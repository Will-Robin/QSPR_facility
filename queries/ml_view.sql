-- create a view for an ML ready subset
CREATE VIEW IF NOT EXISTS surfpro_dataset_v1 AS
...
SELECT -- 4. Pivot the table -> one compound, multiple properties
    ...
    -- derived properties
    -LOG10(MAX(value) FILTER (WHERE property = 'CMC')) AS pCMC,
    -LOG10(MAX(value) FILTER (WHERE property = 'C20')) AS pC20,
